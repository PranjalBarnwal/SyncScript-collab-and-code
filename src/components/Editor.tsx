...
export default function Editor({ editorConfig }) {
  ...
  const onCodeChange = React.useCallback((val) => {
    setValue(val);
  }, []);
  ...
  return (
    <fieldset className="pb-2 flex font-poppins items-center text-primary flex-col h-full w-full border-primary border-[1px] rounded-lg">
      ...
      <CodeMirror minHeight='100vh' theme={ editorThemeMap[editorConfig.theme] } value={ value } extensions={[ editorLangMap[editorConfig.lang]() ]} onChange={ onCodeChange } />
      </div>
    </fieldset>
  );
}
